...
$ python3 sim_space_torus.py run save-folder
...
def create_sim_space(
        gds_fg_name: str,
        gds_bg_name: str,
        sim_width: float = 6000,  # size of sim_space
    ...
    simspace = optplan.SimulationSpace(
        ...
        ), # chaning the size too much creates an error
...
def create_objective(
    ...
    wg_source = optplan.WaveguideModeSource(
        ...
        extents=[GRID_SPACING, wg_width, 600],  # these too
        normal=[1, 0, 0],
        mode_num=0,
        power=1.0,
    )
