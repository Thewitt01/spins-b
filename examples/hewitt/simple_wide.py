...
def create_sim_space(
    ...
    if visualize:
        ...
        eps_fg, eps_bg = get_fg_and_bg(context.get_object(simspace), label=5) #edit here
...
def create_objective(
    ...
    wg_source = optplan.WaveguideModeSource(
        center=[-2750, 0, 0],  # may need to edit these, not too sure
        extents=[GRID_SPACING, 3000, 600],  # these too # waveguide overlap should be larger
        normal=[1, 0, 0],
        mode_num=0,
        power=1.0,
    )
    ...
    wg_out = optplan.WaveguideModeOverlap(
        center=[2750, 0, 0],
        extents=[GRID_SPACING, 3000, 600], #edits to the width
        normal=[1, 0, 0],
        mode_num=0,
        power=1.0,
    )
