...
def run_opt(save_folder: str, sim_width: float) -> None:
    ...
    trans_list = create_transformations(
        obj, monitors, sim_space, cont_iters=400, min_feature=100)
...
def create_sim_space(
    ...
    # Switch x and y temporarily to try and get better direction for field
    ...
    simspace = optplan.SimulationSpace(
        ...
        sim_region=optplan.Box3d(
            center=[0, 0, 0],
            extents=[5000, 5000, 40], # this is what's messing things up, needs to be 2D
    ...
    if visualize:
        ...
        eps_fg, eps_bg = get_fg_and_bg(context.get_object(simspace), wlen=780)
...
def create_objective(
    ...
    wg_source = optplan.WaveguideModeSource(
        center=[-2250, 0, 0],  # may need to edit these, not too sure
    ...
    annulus = optplan.AnnulusOverlap(
        normal=[1, 0, 0], # does it really have a normal?
        mode_num=0,
        power=1.0,
    )
    ...
    monitor_list = []
    for wlen, overlap in zip([780], [annulus]):
