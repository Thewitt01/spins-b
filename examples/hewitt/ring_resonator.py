...
def create_sim_space(
    ...
    waveguide_bottom = gdspy.Rectangle((-1.1 * sim_size[0] / 2, (-box_width/ 2)-wg_width),
                                (1.1 * sim_size[0] / 2, -box_width / 2),
                                LAYER)

    design_region = gdspy.Rectangle((-box_width / 2, -box_width / 2),
                                    (box_width / 2, 0), # extend region?
                                    LAYER)
    annulus = gdspy.Round(
        (0, 1000),
        1000,
...
def create_objective(
    ...
    monitor_list = []
    for wlen, overlap, label in zip([1070, 1070], [upper, wg_out], [1, 2]):
